...
| A08 Software or Data Integrity Failures | Addressed | API independently validates raw AI extraction output against the embedded shared schema before typed binding and state transition; the independent deterministic rules then decide `NEEDS_REVIEW` versus posting. Atomic status transitions and idempotency prevent duplicate posting. Evidence: `ExtractionProposalContractTest`, `ExtractionProposalParsingTest`, `DocumentStatusPipelineIT`, `ProposalContractConformanceTest`, `apps/ai/tests/test_contract.py`, `ExpensePostingPipelineIT`, and `ExpenseReviewIT`. |
